...
async function main() {
  ...
  if (currentDatum.member_count === 0n) {
    ...
    console.log(
      "member_count=0, is_active=true → deactivating group for deletion.",
    );
  } else {
    // Members still in the group — deactivate to signal exit window (all exits become penalty-free).
    console.log(
      `member_count=${currentDatum.member_count} — deactivating group (is_active: true → false).`,
    );
    console.log(
      "NOTE: deactivation is irreversible — all future exits will be penalty-free (mature path).",
    );
    updatedDatum = { ...currentDatum, is_active: false };
  }
