...
import { accountPolicyId, groupPolicyId, assetNameLabels } from "@dcu/sdk";
...
const GRAVEYARD_KEY_HASH = "4443555f4752415645594152445f5631000000000000000000000000";
...
async function main() {
    ...
    if (isEmulator) {
        console.log("purge-nfts targets live networks only — stale tokens don't accumulate in the emulator.");
        process.exit(0);
    }
    ...
    console.log(`Active account suffix : ${activeAccountSuffix ?? "(none in state.json)"}`);
    console.log(`Active group suffix   : ${activeGroupSuffix   ?? "(none in state.json)"}`);
    ...
    const allUtxos = (await lucid.wallet().getUtxos()).filter(u => !u.scriptRef);
    ...
    for (const utxo of allUtxos) {
        for (const unit of Object.keys(utxo.assets)) {
            ...
            const isStaleAccount = policyId === accountPolicyId && suffix !== activeAccountSuffix;
            const isStaleGroup   = groupPolicyId != null && policyId === groupPolicyId && suffix !== activeGroupSuffix;
    ...
    if (staleUnits.length === 0) {
        ...
        if (allUtxos.length > 1) {
            console.log(`(${allUtxos.length} UTxOs present — run with CONSOLIDATE=true to consolidate ADA anyway)`);
        }
        return;
    }
    ...
    for (const unit of staleUnits) {
        ...
        const assetName = unit.slice(56);
        const suffix    = assetName.slice(8);
        const kind      = policyId === accountPolicyId ? "AccountUser(222)" : "GroupAdmin(222)";
    ...
    console.log(`Collecting all ${allUtxos.length} wallet UTxOs → consolidated ADA + active tokens return as change.\n`);
    ...
    console.log(`Done! ${staleUnits.length} stale token(s) swept. Wallet UTxOs consolidated.`);
