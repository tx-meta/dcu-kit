...
async function main() {
    ...
    if (isEmulator) {
        console.log("send-ada requires a live network (set BLOCKFROST_KEY or MAESTRO_API_KEY in .env).");
        process.exit(0);
    }
    ...
    const toAddrLucid = await (() => {
        ...
        if (maestroKey) {
            return Lucid(new Maestro({ network, apiKey: maestroKey, turboSubmit: false }), network);
        }
    ...
    const tx      = await lucid.newTx().pay.ToAddress(toAddress, { lovelace: amount }).complete();
