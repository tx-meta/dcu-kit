...
# ── 1. Deploy reference scripts (once per SDK version, permanent) ─────────────
pnpm run deploy-scripts
# → deploys SIX ref scripts one per tx (treasury dispatcher, group, and the four
#    family stake validators), saving every outRef to state.json
# → registers the FOUR family stake credentials — the withdraw-zero prerequisite;
#    no treasury operation can run without them
...
### Escrow v2 + Project + Pool (versioned — `@tx-meta/dcu-kit/escrow/v2`)

v2 is a NEW validator generation beside v1 (never a replacement): live v1
escrows keep finishing on the v1 hash. Parties are given as plain addresses or
`.env` wallet names (`ADMIN`/`USER1`/`USER2`) — nobody types a credential hash.
Live sweep completed on Preprod 2026-07-11 — every endpoint below is
chain-proven.

**One-time step:** `escrow-v2-deploy` parks the 11.4 KB v2 script in a
reference-script UTxO at the deployer's own address (~50 ADA min-ADA,
recoverable). `pool-allocate` requires it — the allocation transaction
