...
const SUBMIT_COOLDOWN_MS =
  Number(process.env.SUBMIT_COOLDOWN_SECS ?? 120) * 1_000;
...
async function makeLucid(): Promise<LucidEvolution> {
  ...
  throw new Error(
    "Set BLOCKFROST_KEY or MAESTRO_API_KEY in .env to run the cron daemon.",
  );
}
...
async function checkRound(
  ...
  if (!groupUtxo) {
    return {
      ready: false,
      reason: "Group UTxO not found on-chain. Has the group been created?",
    };
  }
  if (!groupUtxo.datum) {
  ...
  if (!groupDatum.is_started) {
    return {
      ready: false,
      reason: "Group has not been started — run start-group first.",
    };
  }
...
async function tick(
  lucid: LucidEvolution,
): Promise<{ submitted: boolean; sleepMs: number }> {
  ...
  if (!scriptRefs) {
    console.warn(
      "[cron] No reference scripts found in state.json — using inline scripts (may approach 16KB limit).",
    );
  }
  ...
  try {
    ...
    if (String(e).includes("OutsideValidityInterval")) {
      console.warn(
        "[cron] OutsideValidityInterval — network slot lags local clock. Retrying after poll interval.",
      );
      return { submitted: false, sleepMs: POLL_INTERVAL_MS };
    }
...
async function main(): Promise<void> {
  ...
  if (total < 5_000_000n) {
    console.warn(
      "[cron] WARNING: wallet balance below 5 ADA — may not have enough for collateral.",
    );
  }

  process.on("SIGINT", () => {
    console.log("\n[cron] SIGINT received — stopping after current poll.");
    stopping = true;
  });
  process.on("SIGTERM", () => {
    console.log("\n[cron] SIGTERM received — stopping after current poll.");
    stopping = true;
  });
