...
import { nextCycle, NextCycleConfig, groupPolicyId, accountPolicyId } from "@dcu/sdk";
...
async function main() {
    ...
    if (isEmulator) {
        console.log("This example requires an existing on-chain group in mature state.");
        console.log("Run on Preprod with distribute-payout completing all rounds first.");
        process.exit(0);
    }
    ...
    console.log("  1. Members re-deposit:  ACTIVE_WALLET=<wallet> TOP_UP_AMOUNT=<lovelace> pnpm run contribute");
