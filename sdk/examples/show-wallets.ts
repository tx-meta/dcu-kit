...
import { groupPolicyId, accountPolicyId, assetNameLabels } from "@dcu/sdk";
...
function labelToken(policyId: string, unit: string, knownGroupPolicy: string | undefined, knownAccountPolicy: string | undefined): string {
    ...
    if (policyId === knownGroupPolicy) {
        kind = prefix4 === assetNameLabels.prefix100 ? "GroupRef(100)" :
               prefix4 === assetNameLabels.prefix222 ? "GroupAdmin(222)" : `GroupToken(${prefix4})`;
    } else if (policyId === knownAccountPolicy) {
        kind = prefix4 === assetNameLabels.prefix100 ? "AccountRef(100)" :
               prefix4 === assetNameLabels.prefix222 ? "AccountUser(222)" : `AccountToken(${prefix4})`;
    } else {
        kind = "UnknownToken";
    }
...
async function showWallet(
    ...
    for (const utxo of utxos) {
        for (const [unit, qty] of Object.entries(utxo.assets)) {
            ...
            if (policyId === groupPolicyId || policyId === accountPolicyId) {
                dcuTokens.push(labelToken(policyId, unit, groupPolicyId ?? undefined, accountPolicyId ?? undefined));
            } else {
    ...
    console.log(`│  ADA     : ${lovelaceToAda(totalLovelace)} ADA  (${utxos.length} UTxO${utxos.length !== 1 ? "s" : ""})`);
...
async function main() {
    ...
    if (!network) {
        console.log("show-wallets requires a live network (NETWORK=Preprod in .env).");
        process.exit(0);
    }
    ...
    if (blockfrostKey) {
        ...
        lucid = await Lucid(new Maestro({ network, apiKey: maestroKey, turboSubmit: false }), network);
    } else {
        console.error("No provider configured. Set BLOCKFROST_KEY or MAESTRO_API_KEY in .env");
        process.exit(1);
    }
