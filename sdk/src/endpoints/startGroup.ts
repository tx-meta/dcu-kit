...
export const unsignedStartGroupTxProgram = (
  ...
  Effect.gen(function* () {
    ...
    const rawNow = config.currentTime !== undefined
        ? config.currentTime
        : BigInt(Date.now()) - 120_000n;
    const now = config.currentTime !== undefined
        ? rawNow
        : rawNow - rawNow % 1000n;
    ...
    const tx = yield* lucid
        ...
        .completeProgram()
        .pipe(Effect.mapError(e => new TransactionBuildError({ operation: "startGroup", error: String(e) })));
