...
import { treasuryValidator, treasuryPolicyId, accountPolicyId } from "../core/validators/constants.js";
...
export const unsignedDeferRoundTxProgram = (
  ...
  Effect.gen(function* () {
    ...
    if (!('TreasuryState' in treasuryDatum)) {
        return yield* Effect.fail(new InvalidDatumError({ field: "treasuryDatum", reason: "Expected TreasuryState for DeferRound" }));
    }

    const ts = treasuryDatum.TreasuryState;

    if (ts.is_deferred) {
        return yield* Effect.fail(new InvalidDatumError({ field: "is_deferred", reason: "Round is already deferred — idempotent guard" }));
    }
    ...
    const tx = yield* lucid
        ...
        .completeProgram()
        .pipe(Effect.mapError(e => new TransactionBuildError({ operation: "deferRound", error: String(e) })));
