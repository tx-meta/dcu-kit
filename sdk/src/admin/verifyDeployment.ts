...
export const verifyDeployment = (
    ...
    Effect.gen(function* () {
        ...
        if (!treasury) {
            issues.push(`Treasury ref UTxO not found: ${treasuryRef.txHash}#${treasuryRef.outputIndex}`);
        } else {
            if (treasury.address !== deployAddress)
                issues.push(`Treasury UTxO is at wrong address: ${treasury.address}`);
            if (!treasury.scriptRef)
                issues.push("Treasury UTxO has no scriptRef");
            else if (treasury.scriptRef.script !== treasuryValidator.mintTreasury.script)
                issues.push("Treasury scriptRef CBOR does not match current compiled validator — contracts upgraded?");
        }

        if (!group) {
            issues.push(`Group ref UTxO not found: ${groupRef.txHash}#${groupRef.outputIndex}`);
        } else {
            if (group.address !== deployAddress)
                issues.push(`Group UTxO is at wrong address: ${group.address}`);
            if (!group.scriptRef)
                issues.push("Group UTxO has no scriptRef");
            else if (group.scriptRef.script !== groupValidator.spendGroup.script)
                issues.push("Group scriptRef CBOR does not match current compiled validator — contracts upgraded?");
        }
