...
export const getWalletUtxos = (
  ...
  Effect.tryPromise({
    try: () => lucid.wallet().getUtxos(),
      ...
      new LucidError({ message: "Failed to get wallet UTxOs", cause: error }),
  });
...
export const getWalletAddress = (
  ...
  Effect.tryPromise({
    try: () => lucid.wallet().address(),
      ...
      new LucidError({ message: "Failed to get wallet address", cause: error }),
  });
...
export const getUtxosAt = (
  ...
  Effect.tryPromise({
    try: () => lucid.utxosAt(address),
      ...
      new LucidError({
        message: `Failed to get UTxOs at ${address}`,
        cause: error,
      }),
  });
...
export const findGroupReferenceUtxo = (
  ...
  Effect.tryPromise({
    try: async () => {
      ...
      return groupUtxos.find((u) =>
        Object.keys(u.assets).some((k) => k.includes(groupName)),
      );
    },
    catch: (error) =>
      new LucidError({
        message: "Failed to find Group Reference UTxO",
        cause: error,
      }),
  });
