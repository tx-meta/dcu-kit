...
Status: PREPROD RETEST PENDING — tracked evidence is complete except for the
B15/B19 npmjs prerelease retest and the D1 prior-sweep citation closure.
...
| B11 | claim-payout (Pull mode group) | earmark withdrawn | **DONE day 2 (2026-07-20)** on Pull group `0a03ce11…`: create/join×2/start/distribute→earmark 10 ADA into borrower treasury/claim `64b16851…`. Evidence `sdk/examples/evidence/p2-day2-live-rows.json`. | ☑ |
| B12 | update-payout-credential | credential rotated | **DONE day 2** on re-sealed group `cb57d564…` member `1053643c…`: tx `69dbb500…` (credential re-derived+verified from signer). | ☑ |
| B13 | extend-grace-window / terminate-default | grace then removal | **DONE day 2** on group `67e271ed…` (grace 90s): under-funded slot-1 join → distribute `314c5b98…` → DefaultState → extend-grace `8e6bf462…` (grace_extensions_used=1) → terminate-default `f236ffc0…` (member burned, collateral forfeited). | ☑ |
| B14 | begin-recommit → free exit during window → re-seal. | era advances | **CLOCK B, mostly done day 1**: start_time 2026-07-17T12:21:02Z; rotation lap complete (B9/B10); begin-recommit `566a9ddb…` opened the window; free exit `3585aa0f…` + window join `652036d5…` both ran INSIDE the window (member_count back to 2). ONLY the re-seal (start-group) remains, valid ≥2026-07-18T12:21:02Z (day 2). **DONE day 2 (2026-07-20): re-seal `8ac3ead6…`, new era — is_started=true, num_rounds=2, start_time 2026-07-20T15:36:23Z.** | ☑ |
| B15 | propose-recovery (day 1) → approve → execute. Datum stays `recovery_timelock = 86_400_000`; ACT at least 25h after PROPOSAL time. | identity rotated N→N' | **CLOCK A ARMED day 1**: propose `f055c0d1…` (USER1 `b314ee9c…` lost → USER2 `ad864b57…` recoveree); approval collected at propose time (APPROVER_WALLETS=ADMIN), RecoveryRequest.approvals=[ADMIN 18b47e8e…], quorum=1 met. `earliest_execution_slot=1784374635000` → **execute valid 2026-07-18T11:37:15Z (day 2)**. Snapshot `sdk/examples/evidence/p2-recovery-group-snapshot.json` | ⊘ npmjs retest |
| B15-note | | | **Blocked on frozen 0.5.5 by an offchain multi-group resolution bug** (not a validator issue). Both the request token `ad864b57…` and member token `b314ee9c…` resolve ambiguously because both accounts are also members of group `88608c45…` (day-1 reuse). The fix is included in `0.5.6-preprod.0`: exit/recovery resolve treasury states by `group_reference_tokenname`, with exact multi-group emulator regressions green. Final proof must use the npmjs `preprod` artifact, not a local tarball. | ⊘ npmjs retest |
...
| B18 | assign-admin (incl. script-held admin variant) | admin token moved | **DONE day 2** on `88608c45…`: group admin (222) moved USER1→ADMIN, `bc949221…` (one-way transfer, destination verified). Script-held variant covered by the endpoint's destination-script proof path (not separately live-run). | ☑ |
| B19 | exit-group (mature) → terminate-group → delete-group | group burned, bond returned | **PARTIAL**: exit-group proven live — USER2 exited `88608c45…` (`8ae97427…`); B18 done. The group-scoped exit fix is packaged in `0.5.6-preprod.0` and regression-tested. Complete the remaining member exit and delete-group using the npmjs `preprod` artifact. | ⊘ npmjs retest |
| B20 | delete-account | account pair burned | **DONE day 2**: burned spare account `e2e1808e…` (membership-free, from the day-1 lifecycle test), `b4520ed7…`. | ☑ |
...
| D4 | **day 2 / low priority** — the cefebe1 sorted-index fix is emulator-tested; its sibling (7fe15be) is now live-proven in D3; and D2 proves the escrow bytecode is byte-identical to the 2026-07-11 sweep where pool-allocate (both modes) was already live-proven. Re-proving needs escrow-v2-deploy + pool deposit + bounded anchor retries for low incremental assurance. Kept for day 2 if time permits. ORIG:  SDK smoke: pool-allocate with a **nonzero** computed `pool_ref_index` — if the anchor sorts first, the old hardcoded 0 would also pass. Against the deployment's FIXED reference txids, a fresh anchor's sort position is NOT ~50% (it depends on where the fixed txid sits in the 256-bit range): bound attempts at EIGHT, re-creating the anchor each time; if none sorts nonzero, FAIL the smoke explicitly rather than retrying indefinitely. Record every attempt's out-refs and sorted order, the final computed index, and recover/close unused anchors where possible (record any that cannot be). | one live run with index > 0, bounded attempts | ⊝ DEFERRED (low assurance: emulator-tested, sibling live in D3, escrow bytecode byte-identical to the 2026-07-11 sweep where pool-allocate both modes was already live-proven) |
