...
| B11 | claim-payout (Pull mode group) | earmark withdrawn | day 2 — needs a dedicated Pull-mode group (create/join/start/distribute→earmark/claim); standard flow, emulator-covered | ☐ (day 2) |
| B12 | update-payout-credential | credential rotated | day 2 — quick single op on the recommit group after re-seal | ☐ (day 2) |
| B13 | extend-grace-window / terminate-default | grace then removal | day 2 — needs a defaulting-member scenario (member skips a contribution → DefaultState → grace → terminate) | ☐ (day 2) |
...
| B18 | assign-admin (incl. script-held admin variant) | admin token moved | day 2 — on the disposable pre-join group `88608c45…` (must not move the clocks' admin tokens) | ☐ (day 2) |
| B19 | exit-group (mature) → terminate-group → delete-group | group burned, bond returned | day 2 — teardown on the disposable group `88608c45…` (deactivate → members exit → delete); never on the two clock groups | ☐ (day 2) |
| B20 | delete-account | account pair burned | day 2 — on a spare account after its group memberships are cleared | ☐ (day 2) |
